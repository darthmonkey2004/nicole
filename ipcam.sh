...
if [ -f "$HOME/ipcam.conf" ]; then
	...
	echo "IP=$ipcam_ip, PORT=$ipcam_port, CAPTURE_STREAM=$ipcam_cap"
else
	echo "Conf file not found! Using default values..."
	ipcam_ip="192.168.2.9"
	ipcam_port=8080
	ipcam_width=640
	ipcam_height=480
	ipcam_fps=5
...
CAPTURE_STREAM="$ipcam_cap_stream"
...
start_iw_server() {
    send_intent com.pas.webcam.pro/com.pas.webcam.Rolling
    sleep 2s
}
